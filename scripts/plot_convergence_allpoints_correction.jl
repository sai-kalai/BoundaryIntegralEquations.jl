# Plot the convergence behavior of the different methods at all points, densely
# sampled, including many points near the boundary
#
...
filter!(result, (k) -> begin
    ...
    if !(order(k.correction) in [Inf, 32, 16])
        return false
    end
...
for key in sort(collect(keys(result.solutions)), by=(k,) -> begin
    ...
    scatterlines!(
        ...
        label="$(string(nameof(key.bdrycond_t))) BC, $(string(nameof(key.approach_t))), $(key.correction)",
        kwargs...,
    )
end
