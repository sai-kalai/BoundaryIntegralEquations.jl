...
function krcoeffs(derivative_order::Int, accuracy_order::Int)::Vector{<:Real}
...
function compute_coeffs_first(k::Int)::Vector{Real}
    ...
    # Build matrix A (k×k)
    A = [2 * xi^p for p in powers, xi in x]

    b = zeros(k)
    b[1] = 1.0

    cd = A \ b                          # solve linear system
    return cd
end
...
function compute_coeffs_second(k::Int)::Vector{Real}
    ...
    setprecision(BigFloat, 400) do
        ...
        A = [BigFloat(2) * xi^p for p in powers, xi in x]

        A[1, 1] = BigFloat(1)

        b = zeros(BigFloat, k + 1)
        b[2] = BigFloat(2)
